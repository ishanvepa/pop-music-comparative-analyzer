from music_writer import BBChart
...
url = []
workbook = xlsxwriter.Workbook('copy_billboard_top_10_data.xlsx')
playlist = []
...
for x in range(len(year_num)):
    ...
    #put data in BBChart array
    playlist.append(BBChart(rankings_list, songs_list, artist_list))#, chords_list[i], bpm_list[i]))
...
wb = pd.read_excel('copy_billboard_top_10_data.xlsx', sheet_name="2020 Charts")
tempo = []
chords = []
tonic = []
key = []
...
for x in range(10):
    tempo = wb["Tempo"].iloc[x]

print(tempo)

#test idkaisoljhdg lol
#print(playlist[4].get_song()[4])
